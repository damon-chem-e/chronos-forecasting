...
def agg_relative_score(model_csv: Path, baseline_csv: Path):
    ...
    dominick_filtered_baseline_df = baseline_df.loc[baseline_df.index == "dominick"]
    relative_score = model_df.drop("model", axis="columns") / dominick_filtered_baseline_df.drop(
        "model", axis="columns"
    )
